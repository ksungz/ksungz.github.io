...
- GeekNews Markdown의 `Topic Body`를 우선 사용하고 댓글을 제외해 말줄임 없는 장문 근거 확보
...
- 검증 전에 원문에 없는 숫자가 포함된 문장과 실제 본문에 존재하지 않는 근거 인용을 결정론적으로 제거
- 별도 LLM 검증 단계에서 사실 근거·명확성·구체성·실용성·중복도를 평가하고 기준 미달 시 최대 두 번 자동 재작성
...
- 검증 실패 글을 다음 수집에서 최대 3회 재처리하고 실행당 2건을 소스별 순환 방식으로 검증
- 실행 상한 때문에 대기한 글은 실제 분석 전까지 재시도 횟수를 소모하지 않도록 큐 상태 보정
...
- 근거·편집 게이트 단위 테스트 11건, TypeScript, Feed 관련 ESLint 통과
- 운영 글 `14973` 단건 v3 백필 dry-run에서 기존 관련도 86점과 무관하게 근거·편집 검증 미달을 감지해 공개 보류 확인
- 기존 공개 14건 전체 v3 백필은 전부 새 품질 기준을 통과하지 못해 안전장치가 데이터 변경을 차단했고 기존 공개 상태를 유지
- GitHub Actions 수집 실행 `29158680358`, `29159110683` 성공 및 v3 자동 공개 글 `15021`의 근거 95점·편집 87점·`verification:passed` 확인
- 운영 공개 피드는 기존 v2 글 14건과 새 v3 검증 통과 글 1건으로 총 15건이며, 이후 자동 공개는 v3 검증 통과 글만 허용
